...
async function getCompletionsFromOpenAi(prompts:string, n: number) {
  try {
    const payload = {
  ...
  console.log("yooooo");
  console.log(completionsFromApi.data.choices);
...
export async function POST(req: NextRequest, res: NextResponse) {
  ...
  if (!reqJson.items) {
    ...
    const {items, prompts, n} = reqJson as {items: {id: string}[], prompts: string, n: number};
    if (items.length === 0) {
      return NextResponse.json([], {
        status: 200
      })
